...
| `link_opened` | `domain` (hostname only; the URL scheme — `maps`, `comgooglemaps`, `geo` — for maps-app links), `context` (`organizer_instagram` \| `research_banner` \| `event_share` \| `event_more_information` \| `banner_ad` \| `event_location` \| `newsletter_list` \| `newsletter_post` \| `null`), `target` (`sheet` — in-app browser sheet \| `browser` — the user's default browser via `Linking.openURL`; the Newsletter surface always uses `browser`), `provider` (`apple` \| `google` \| `system` — Android's default `geo:` handler — only with `event_location`; emitted only after the launch succeeds, so a fallback counts once) | `lib/openLink.ts` — every external link; `lib/openInMaps.ts` for the Event Details venue tap |
